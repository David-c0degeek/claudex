...
| D010 | 2026-07-16 | New-run allocation is last in startup preflight | Validate the substantive task, Git repository, ignored/untracked runtime boundary, clean exact base, and non-billable provider capabilities before creating a run ID, state/current files, provider process, or watcher windows. | Field use showed INIT previously detected a missing task/dirty base only after Claudex had created artifacts and views, allowing an absent ignore rule to make Claudex reject its own writes. Promoted to permanent architecture/decision docs and released in 0.5.1. | 06.3, 06.5–06.7; `cli.py`, `phases.py`, `gitops.py`, `docs/decisions.md` |
