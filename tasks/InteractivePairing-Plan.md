...
| D022 | 2026-07-26 | Plan-agnostic gate: triage rule + accepted violations | The §7 plan-agnostic items are RULES WITH A DEFINED TRIAGE, not a raw grep. (a) **Shipped code/tests/comments/identifiers**: the grep's `\bslices?\b` and `\b\d\d\.\d+\b` patterns match Go's own vocabulary (`slices` stdlib package, `slice` type) and version strings (`git version 2.54.0`); these are named FALSE-POSITIVE CLASSES and are triaged out. What counts is a reference only meaningful with the plan in hand — a box id (`03.9`, `slice-ad`), a decision id, a plan file path, or 'this slice' meaning a plan slice. Two such references existed and were removed; the item is now satisfiable and SATISFIED. (b) **Commit messages**: plan-agnostic SUBJECTS are required going forward. Bodies may cite decisions and boxes, because traceability from code to rationale is worth more than purity and the body is not the artifact a reader of `git log --oneline` sees. Already-pushed subjects that violate this (M10's ~12, plus the 04.5 series) are an ACCEPTED VIOLATION: the plan forbids rewriting pushed history, and CX has reviewed against those exact SHAs. (c) **Branch name**: `interactive-pairing` is an ACCEPTED VIOLATION for the life of the branch — renaming mid-flight would break the peer-review mailbox protocol and every recorded resume pointer for no correctness gain. The PR title and merge commit must be plan-agnostic. | Gate items that cannot be satisfied as literally written get ignored at gate time, which is worse than having no gate; M10 explicitly warned against deciding this at the gate. Deciding it now makes the items checkable and records what was knowingly accepted rather than silently passed | §7 gate items 3-5; `findings.md` M10; `cmd/claudex/pull_test.go`, `internal/coordinator/coordinator_test.go`; `docs/decisions.md` |
...
- [x] Shipped code/tests/comments/identifiers plan-agnostic — grep excluding `tasks/` for box IDs (`\b\d\d\.\d+\b`), decision IDs (`\bD\d{3}\b`), `tasks/interactive-pairing/`, `InteractivePairing-Plan.md`, `\bslices?\b`; zero hits **after the D022 triage** (Go's `slices`/`slice` vocabulary and version strings are named false-positive classes; two genuine references removed)
- [ ] Commit message SUBJECTS plan-agnostic going forward — `git log <base>..HEAD`; pre-D022 subjects are an accepted violation (D022b), bodies may cite decisions/boxes
- [ ] PR title / merge commit plan-agnostic; the branch name `interactive-pairing` is an accepted violation for the life of the branch (D022c)
