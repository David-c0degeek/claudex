...
- [ ] **00.1** (agent) Read repo instructions + authoritative docs. `README.md` is present on `main`; `docs/architecture.md`, `docs/decisions.md`, `tasks/lessons.md`, `tasks/claudex-refactor/`, `CHANGELOG.md` exist **only in refactor-branch history** — read them via `git show reliability-observability-refactor:<path>`. List the intended pair-programming direction + constraints so later subjects flag drift. Record the vision→code gap (README describes the loop; the Python code drove subprocesses).
...
- [ ] **00.8** (agent) **Create** the Go-era `docs/decisions.md` + `docs/architecture.md` (they don't exist on `main`; the refactor-branch versions are input via `git show`, not a base to edit) capturing **D001–D015** + the transport/state/git-transaction contract + the **legacy inspect/export-only** behavior (D011). Start a fresh `CHANGELOG.md` and `tasks/lessons.md` for the Go era. Bake findings into §2/§6/§7, subject boxes, and lessons. Inventory doc surfaces (README rewrite, architecture doc, skill docs). End with an implementation-readiness summary.
