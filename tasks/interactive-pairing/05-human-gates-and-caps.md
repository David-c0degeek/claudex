...
- **Do not duplicate** — no second guidance store; reuse the lifecycle states.
- **Integration point + why** — gates live in `state.py` Phase + `cli.py` resolve/continue; cap policy is new and small, layered over 03's counters.
- **Vision fit** — realizes D004 (gate identity), D007 (observable-only caps), and "human decisions are never silent."
- **Risks** — pretending to enforce token/cost caps in attach (dishonest); two humans answering different gate generations; `status` secretly reconciling instead of projecting `recovery_pending` (must stay read-only, per 01.6).
