...
- **Closure dependency from 01.7 (legacy refusal seam)** — 01.7 shipped `legacy.CheckRunDir(dir)`, a read-only guard returning a typed `*legacy.LegacyRunError` (carrying `Remediation`) when a target directory holds a pre-pivot Python `state.json`. The first-attach bootstrap (03.1) **must** call it before any allocation/resume mutation and fail closed with the remediation, so a legacy run directory is never allocated over or silently treated as an empty attach store. This is the wiring that lets 01.7 claim end-to-end resume/execution refusal; until it lands, 01.7's refusal is seam-only.
