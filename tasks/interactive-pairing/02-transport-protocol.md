...
- 2026-07-16 · slice 3 · submit (02.3) · `internal/transport.Submit(ctx, store, sink, session, raw, authorize, advance)` — accept-once under the state CAS. Reconstructs authority from durable state (never client metadata), authorizes every path (incl. replay), redacts then canonicalizes for a digest so a secret-only difference collapses idempotently, validates raw-then-redacted against the registry schema, persists the immutable artifact to an injected sink BEFORE the CAS, and records acceptance + the caller's transition in one mutation. Three paths: same turn+digest → idempotent receipt, same turn+different → conflict, superseded revision → typed `StaleError` with current status; a lost CAS race reclassifies; a committed `PostCommitError` reconciles to the receipt; busy-exhaustion returns typed busy (never stale). The transition gets an immutable `PreparedSubmit`, runs first, and `requireLiveOwner` enforces the mutation left a real next owner (running agent turn / running TESTS / paused issued gate / terminal-with-projection) — no stranded shape. Authority facts captured before the authorizer so it can't fabricate acceptance. CX AGREE (`8e1b857`,`c67ce75`,`81a2ef5`,`cb97381`, 5 rounds — caught record-only crash-cut, artifact-after-state ordering, optional auth, busy-as-stale, prepared-artifact side channel, mutable prepared bytes, secret-leaking parse errors, authorizer aliasing, and the ownerless-owner shapes).
