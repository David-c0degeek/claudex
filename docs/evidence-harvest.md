...
- **`PLAN_CRITIQUE`** — `verdict∈{AGREE,REVISE}, findings[PLAN_FINDING], implementation_checks[], missing_evidence[], simpler_alternative:string|null, notes`. (In the Go protocol, `requires_human_decision`/`decision_question` move OUT of this payload into the common envelope — see the schema-delta below.)
...
- **Human-decision truth lives in ONE place: the envelope.** The envelope owns routing identity + `human_context` + the decision request for *every* phase. Phase payloads (`PLAN_CRITIQUE`, `CHECKPOINT_REVIEW`, …) therefore **do not** carry their own `requires_human_decision`/`decision_question` — those move out of the payloads into the envelope. This removes the duplicate source of truth so strict validation can never accept an envelope and a payload that disagree. The `ProtocolViolation` consistency check (flag XOR question) is evaluated once, on the envelope.
...
- **`CHECKPOINT_REVIEW` must express missing evidence** — extend it with `missing_evidence[]` so a reviewer can be inconclusive, mirroring `PLAN_CRITIQUE` (today it cannot). The human-decision request stays **envelope-only** (see above) — it is NOT added to this payload.
