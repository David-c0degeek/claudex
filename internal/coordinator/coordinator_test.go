...
// fakeTestRunner is a deterministic TestRunner: the coordinator never execs a real command.
...
func (f *fakeTestRunner) RunTests(_ context.Context, command string) (bool, []byte, error) {
	f.calls++
	f.lastCmd = command
	return f.pass, f.evidence, f.err
}

func policyEnabledGate(command string) []byte {
	p := config.DefaultRunPolicy()
	p.TestGate = config.TestGate{Command: command}
	p.Budgets.PlanRounds = 1
	p.Budgets.CheckpointRounds = 3
	b, _ := json.Marshal(p)
	return b
}

// An enabled gate with a passing runner drives ownerless TESTS to VERIFY; the runner is
// invoked with the gate command and its evidence backs the committed outcome.
func TestRunTestsEnabledPassToVerify(t *testing.T) {
	repo := t.TempDir()
	runID, lead, pair := newPairedRunWithPolicy(t, repo, policyEnabledGate("run-the-tests"))
	rn, err := OpenRun(repo, runID, rand.Reader)
	if err != nil {
		t.Fatalf("open run: %v", err)
	}
	...
	fr := &fakeTestRunner{pass: true, evidence: []byte("all green")}
	res, err := rn.RunTests(context.Background(), fr)
	if err != nil {
		t.Fatalf("run tests: %v", err)
	}
	if fr.calls != 1 || fr.lastCmd != "run-the-tests" {
		t.Fatalf("runner not invoked with the gate command: calls=%d cmd=%q", fr.calls, fr.lastCmd)
	}
	if res.Revision == 0 {
		t.Fatal("no committed outcome revision")
	}
	if cur(t, rn).Phase != state.PhaseVerify {
		t.Fatalf("passing gate did not enter VERIFY: %s", cur(t, rn).Phase)
	}
}

// An enabled gate with a failing runner leaves the ownerless TESTS phase (to FIX / the budget gate).
func TestRunTestsEnabledFailLeavesTests(t *testing.T) {
	repo := t.TempDir()
	runID, lead, pair := newPairedRunWithPolicy(t, repo, policyEnabledGate("run-the-tests"))
	rn, err := OpenRun(repo, runID, rand.Reader)
	if err != nil {
	...
	if _, err := rn.RunTests(context.Background(), &fakeTestRunner{pass: false, evidence: []byte("2 failed")}); err != nil {
		t.Fatalf("run tests: %v", err)
	}
	if next := cur(t, rn); next.Phase == state.PhaseTests {
		t.Fatalf("failing gate did not leave TESTS: %s", next.Phase)
	}
}

// A disabled gate is a pass with no mechanical runner.
func TestRunTestsDisabledGatePasses(t *testing.T) {
	repo := t.TempDir()
	runID, lead, pair := newPairedRun(t, repo) // default policy: the gate is disabled
	rn, err := OpenRun(repo, runID, rand.Reader)
	if err != nil {
		t.Fatalf("open run: %v", err)
	}
	...
	if _, err := rn.RunTests(context.Background(), nil); err != nil {
		t.Fatalf("disabled gate: %v", err)
	}
	if cur(t, rn).Phase != state.PhaseVerify {
		t.Fatalf("disabled gate did not pass to VERIFY: %s", cur(t, rn).Phase)
	}
}

// An enabled gate with no runner, or a runner that errors, fails closed and authors no outcome.
func TestRunTestsEnabledGateFailClosed(t *testing.T) {
	repo := t.TempDir()
	runID, lead, pair := newPairedRunWithPolicy(t, repo, policyEnabledGate("run-the-tests"))
	rn, err := OpenRun(repo, runID, rand.Reader)
	if err != nil {
	...
	if _, err := rn.RunTests(context.Background(), nil); err == nil {
		t.Fatal("an enabled gate with no runner must fail")
	}
	if _, err := rn.RunTests(context.Background(), &fakeTestRunner{err: errors.New("runner blew up")}); err == nil {
		t.Fatal("a runner error must propagate")
	}
	if after := cur(t, rn); after.Revision != before.Revision || after.Phase != state.PhaseTests {
		t.Fatalf("a failed gate mutated the run: rev %d->%d phase %s", before.Revision, after.Revision, after.Phase)
	}
}
