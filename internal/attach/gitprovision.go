...
// (".claudex") the attach layout writes under so gitx stays generic.
type gitPreflighter struct{ g *gitx.Git }

// NewGitPreflighter builds the real definite-new-run preflighter over a hardened git handle.
func NewGitPreflighter(g *gitx.Git) Preflighter { return gitPreflighter{g: g} }

func (p gitPreflighter) Preflight(ctx context.Context, repoDir string) error {
	return p.g.Preflight(ctx, repoDir, runtimeDirName)
}
