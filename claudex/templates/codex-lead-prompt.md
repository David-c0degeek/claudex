...
either of you — owns phase order, round caps, and convergence.

Rules the coordinator enforces:
- Convergence = pair verdict AGREE with zero blocking/major findings.
- Hard round caps everywhere. Cap hit → the run gates; bring the open
...
2. Investigate the repo; write plan JSON: `plan_markdown`, `steps`
   ...
   independently implementable), `risks`, `open_questions`.
3. `claudex pair plan --file <plan.json>` — long-running (invokes Claude);
   ...
   FULL plan; submit again until AGREE.
...
`claudex status` always shows the phase, step, round budgets, and the next
command. `mailbox.md` in the run dir is the full transcript.
